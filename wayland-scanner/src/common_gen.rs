...
fn event_method_prototype(name: &Ident, msg: &Message) -> TokenStream {
    ...
    quote! {
        fn #method_name(&self, proxy: Proxy<#name>, #(#method_args),*) {}
    }
}
...
pub(crate) fn gen_event_handler_trait(name: &Ident, messages: &[Message]) -> TokenStream {
    ...
    let method_patterns = messages.iter().map(|msg| {
        ...
        quote! {
            Event::#msg_name { #(#arg_names),* } => {
                (self.0).#method_name(proxy, #(#arg_names_2),*)
            }
        }
    });
    ...
    quote! {
        pub trait EventHandler {
            ...
            /// Wraps this handler in a wrapper suitable for `NewProxy::implement_handler()`.
            #[inline]
            fn wrap(self) -> EventHandlerWrapper<Self>
            where
                Self: Sized,
            {
        ...
        /// Wrapper suitable for `NewProxy::implement_handler()`.
        pub struct EventHandlerWrapper<T: EventHandler>(T);

        impl<T: EventHandler> Handler<#name> for EventHandlerWrapper<T> {
            #[inline]
            fn handle(&self, event: Event, proxy: Proxy<#name>) {
                match event {
                    #(#method_patterns)*
                }
            }
